...
struct RemotePlayInput_t
{
	...
	union
	{
		...
		//char padding[ 64 - ( sizeof( m_unSessionID ) + sizeof( m_eType ) ) ]; //RAXAT: m_unSessionID and m_eType are not part of the union, error C2327 is triggered
		char padding[64 - (sizeof(RemotePlaySessionID_t) + sizeof(ERemotePlayInputType))]; //RAXAT: here's the correct line
	};
};
